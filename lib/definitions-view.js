...
export default class DefinitionsView {
  constructor(emptyMessage = 'No definition found', maxResults = null) {
    this.selectListView = new DefinitionsListView({
      ...
      filterKeyForItem: this.getFilterKey.bind(this),
    ...
    //this.panel = atom.workspace.addTopPanel({ item: this, visible: false });
    this.items = [];
  ...
  elementForItem({ fileName, text, line }) {
    ...
    primaryLine.setAttribute("style","overflow:auto;text-overflow:clip;");
  ...
  async didConfirmSelection({ fileName, line, column }) {
    ...
    await promise.then((editor) => {
      ...
      let PixelPosition = View.pixelPositionForScreenPosition(Cursor).top;
      let LineHeight = editor.getLineHeightInPixels();
      //위에서 두번째줄 여유
      PixelPosition -= (LineHeight * 2);
