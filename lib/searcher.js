...
export default class Searcher {
  ...
  static filterMatch(match) {
    //filename 파일경로에 lang 관련된 문자열이 있으면 필터링
    return (match !== null && match.text.trim().length < 350 && match.fileName.indexOf(`\\${match.lang}\\`) >= 1);
  }
  ...
  static atomWorkspaceScan(activeEditor, fileLang, fileTypes, regex, iterator, callback) {
    // this.atomBufferScan(activeEditor, fileTypes, regex, iterator, (openedFiles) => {
      //paths 에 검색할 패스가 들어간다.
      // atom.workspace.scan(new RegExp(regex, 'ig'), { paths: [`*.${fileLang}.*`] }, (result) => {
      atom.workspace.scan(new RegExp(regex, 'ig'), { paths: [`*`] }, (result) => {
        // if (openedFiles.includes(result.filePath)) {
        //   console.log('열려있는 탭에 검색 결과가 있네요');
